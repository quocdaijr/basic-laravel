@php
    ...
    $userNameField = config('LaravelLogger.defaultUserNameField')
@endphp
            ...
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <select class="form-control" name="user">
                    ...
                    @foreach($users as $user)
                        <option
                            value="{{ $user->$userIdField }}"{{ request()->get('user') && request()->get('user') == $user->$userIdField ? ' selected':'' }}>{{ $user->$userNameField }}</option>
                    @endforeach
                </select>
            </div>
        @endif
            ...
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <select class="form-control" name="method">
                    ...
                    @foreach(explode(' ','CONNECT DELETE GET OPTIONS PATCH POST PUT TRACE') as $val)
                        <option
                            value="{{ $val }}"{{ request()->get('method') && request()->get('method') == $val ? ' selected':''}}>{{ $val }}</option>
                    @endforeach
                </select>
            </div>
        @endif
        @if(in_array('route',explode(',', config('LaravelLogger.searchFields'))))
            <div class="col-12 col-sm-4 col-lg-2 mb-2">
                <input type="text" name="route" class="form-control"
                       value="{{request()->get('route') ? request()->get('route'):null}}" placeholder="Route">
            </div>
        @endif
