...
class BuildTagSidebarListener extends BuildSidebarListener
{
  ...
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                ...
                if (!$item->hasItems()) {
                    ...
                    $item->authorize(
                        $item->isAuthorized() || $this->checkAccess((array)'tag.index')
                    );
                }
                $item->item(__('Tag'), function (Item $item) {
                    $item->icon('<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
</svg>');
                    $item->route('tag.index');
                    $item->isActiveWhen(route('tag.index', null, false));
                    $item->authorize(
                        $this->checkAccess((array)'tag.index')
                    );
                });
