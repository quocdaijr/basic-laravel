                    ...
                    @foreach($tags as $tag)
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-xs">
                                {!! \Modules\Core\Constants\CoreConstant::getHtmlStatus($tag->status) !!}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <div class="flex items-center text-sm">
                                    {{$tag->created_at}}
                            ...
                            <td class="flex flex-wrap">
                                ...
                                {!! Form::open(['method' => 'DELETE', 'route' => ['tag.destroy', $tag->id], 'id' => 'form_delete_' . $tag->id]) !!}
                                <button
                                    class="flex items-center justify-between my-2 mx-1 px-2 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-full active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple dark:text-gray-300"
                                    onclick="alertFormSubmitConfirm('form_delete_{{$tag->id}}', 'Are you sure to delete this!')"
                                    type="button"
                                    aria-label="Delete">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                         fill="currentColor">
