    ...
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <div class="w-full overflow-hidden rounded-lg shadow-xs">
            <div class="w-full overflow-x-auto">
                <a href="{{route('administration.user.create')}}"
                    ...
                    @foreach($users as $user)
                        <tr class="text-gray-700 dark:text-gray-400">
                            ...
                            <td class="px-4 py-3 text-xs">
                                {!! \Modules\Core\Constants\CoreConstant::getHtmlStatus($user->status) !!}
                            </td>
                            <td class="px-4 py-3 text-sm">
                                <div class="flex items-center text-sm">
                                    {{$user->created_at}}
                            ...
                            <td class="flex flex-wrap">
                                <a href="{{route('administration.user.show', $user->id)}}"
                                ...
                                <a href="{{route('administration.user.edit', $user->id)}}"
                                   class="flex items-center justify-between my-2 mx-1 px-2 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-full active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple dark:text-gray-300"
                                   aria-label="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20"
                                         fill="currentColor">
                                ...
                                {!! Form::open(['method' => 'DELETE', 'route' => ['administration.user.destroy', $user->id]]) !!}
                                <button
                                    class="flex items-center justify-between my-2 mx-1 px-2 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-full active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple dark:text-gray-300"
